...
export class KeyvGuildConfigRepository implements IGuildConfigRepository {
    ...
    constructor(connectionString: string) {
        this.keyv = new Keyv(connectionString);
    ...
    async getConfig(guildId: string): Promise<GuildConfig | null> {
        try {
            ...
            throw new DatabaseError(`Failed to get guild config: ${error instanceof Error ? error.message : 'unknown error'}`);
        }
    }
    ...
    async saveConfig(config: GuildConfig): Promise<void> {
        try {
            ...
            throw new DatabaseError(`Failed to save guild config: ${error instanceof Error ? error.message : 'unknown error'}`);
        }
    }
    ...
    async updateConfig(guildId: string, updates: Partial<GuildConfig>): Promise<void> {
        try {
            ...
            throw new DatabaseError(`Failed to update guild config: ${error instanceof Error ? error.message : 'unknown error'}`);
        }
    }
    ...
    async deleteConfig(guildId: string): Promise<void> {
        try {
            ...
            throw new DatabaseError(`Failed to delete guild config: ${error instanceof Error ? error.message : 'unknown error'}`);
        }
    }
    ...
    async getBumpReminderConfig(guildId: string): Promise<BumpReminderConfig | null> {
        const config = await this.getConfig(guildId);
        return config?.bumpReminderConfig || null;
    }

    async updateBumpReminderConfig(guildId: string, bumpReminderConfig: BumpReminderConfig): Promise<void> {
    ...
    async updateStickMessages(guildId: string, stickMessages: StickMessage[]): Promise<void> {
 ...
 * Repositoryインスタンス作成
 */
export const createGuildConfigRepository = (databaseUrl: string): IGuildConfigRepository => {
