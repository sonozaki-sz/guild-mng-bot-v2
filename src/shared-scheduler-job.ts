...
export class JobScheduler {
    ...
    public addJob(job: ScheduledJob): void {
        ...
        try {
            ...
            logger.info(`Job scheduled: ${job.id}${job.description ? ` - ${job.description}` : ''}`);
        } catch (error) {
...
export class BumpReminderManager {
    private reminders: Map<string, { jobId: string; lastRemindedAt?: Date }> = new Map();
    ...
    public setReminder(
        ...
        jobScheduler.addJob({
            ...
            task: async () => {
                try {
                    ...
                    logger.error(`Bump reminder task failed for guild ${guildId}:`, error);
                } finally {
                    // 1回限りなので削除
        ...
        logger.info(`Bump reminder scheduled for guild ${guildId} in ${delayMinutes} minutes`);
    }
