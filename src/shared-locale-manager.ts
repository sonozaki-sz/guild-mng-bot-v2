...
export class LocaleManager {
     ...
     * Guild別の翻訳文字列を取得
     */
    async translate(guildId: string | undefined, key: string, params?: Record<string, any>): Promise<string> {
        try {
            ...
            // @hi18n/core の翻訳メソッドを呼び出す
            // 実際の実装はカタログの構造に依存
            return catalog.t(key, params) || key;
        } catch (error) {
 ...
 * ヘルパー関数：Guild別翻訳
 */
export const t = async (guildId: string | undefined, key: string, params?: Record<string, any>): Promise<string> => {
...
export const tDefault = (key: string, params?: Record<string, any>): string => {
    const catalog = localeManager['catalogs'].get(localeManager.getDefaultLocale());
